...
export function UploadPanel({ onUpload, uploading }: UploadPanelProps) {
  ...
  return (
    <section className="rounded-[2rem] border border-gold/20 bg-[radial-gradient(circle_at_top,#19304f,transparent_60%),linear-gradient(135deg,#101c2d,#0b1320)] p-6 shadow-glow">
      ...
      <button
        ...
        onClick={() => fileRef.current?.click()}
        className={`mt-8 w-full rounded-[1.75rem] border border-dashed p-10 text-left transition ${
          dragging
      ...
      >
        ...
        <span className="mt-3 block font-display text-2xl text-white">
